		...
		<view class="tui-reply-tabbar" :style="{ paddingBottom: `${keyboardHeight}px` }">
			<view class="tui-chat-tabbar">
				...
				<view class="tui-send-box">
					...
					<view class="tui-btn-send" v-if="content" :style="{ color: color }" hover-class="tui-opcity" :hover-stay-time="150" @tap="sendMessage" >{{ sendText }}</view>
				</view>
			</view>
...
export default {
	...
	data() {
		return {
			...
			keyboardHeight: 0,
			replyContainerH: uni.upx2px(400),
			faceList: [],
			content: '',
			isVoice: false
		};
	...
	methods: {
		sendMessage: function() {
			//隐藏键盘
			this.showIndex = 0;
			this.$eventHub.$emit('send', this.content)
			this.content = ''
			// uni.hideKeyboard();
		},
		showKeyBoard(index) {
